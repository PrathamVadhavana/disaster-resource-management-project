...
async def update_volunteer_profile(
    ...
    existing = (
        await db_admin.table("volunteer_profiles").select("id").eq("user_id", user_id).maybe_single().async_execute()
    )
...
async def volunteer_check_in(
    ...
    active = (
        await db_admin.table("volunteer_ops").select("id").eq("user_id", user_id).eq("status", "active").async_execute()
    )
    if active.data:
        raise HTTPException(
            status_code=400,
...
async def volunteer_check_out(
    ...
    resp = await db_admin.table("volunteer_ops").update(updates).eq("id", op_id).async_execute()
...
async def get_volunteer_dashboard_stats(
    ...
    resp = await db_admin.table("volunteer_ops").select("status, hours_worked").eq("user_id", user_id).async_execute()
    ops = resp.data or []
