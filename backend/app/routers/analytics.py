...
async def get_platform_summary(user=Depends(require_role("admin", "ngo"))):
    ...
    try:
        ...
        verif_resp = (
            await db_admin.table("request_verifications").select("verification_status").limit(5000).async_execute()
        )
        ...
        mobilization_resp = (
            await db_admin.table("ngo_mobilization").select("id", count="exact").limit(1000).async_execute()
        )
        ...
        result = {
            ...
            "donations": {"units_pledged": total_units_pledged, "active_pledges": len(pledges)},
            "mobilization": {"total_missions": total_missions},
...
async def get_volunteer_analytics(user=Depends(require_admin)):
    ...
    try:
        ...
        verif_resp = (
            ...
            .select("volunteer_id, verification_status")
            .limit(5000)
            .async_execute()
        )
        verifs = verif_resp.data or []
            ...
            result.append(
                {
                    ...
                    "email": u["email"],
                    **stats,
                    "accuracy": round(stats["trusted"] / stats["total_verifs"] * 100, 1)
                    if stats["total_verifs"] > 0
                    else 0,
                }
            )
...
async def get_resource_burn_rate(days: int = Query(30, ge=1, le=365), user=Depends(require_role("admin", "ngo"))):
    ...
    try:
        ...
        req_resp = (
            ...
            .select("created_at, resource_type, quantity, status")
...
async def get_geospatial_impact(user=Depends(require_admin)):
    ...
    try:
        resp = (
            await db_admin.table("resource_requests")
            .select("id, latitude, longitude, resource_type, priority, status")
...
async def export_interactivity_data(
    table: str = Query(..., pattern="^(verifications|pledges|missions|pulse)$"), admin=Depends(require_admin)
):
    """
    ...
    try:
        ...
        if table == "verifications":
            ...
            for r in rows:
                writer.writerow(
                    [
                        ...
                        r.get("field_notes"),
                        r.get("latitude_at_verification"),
                        r.get("longitude_at_verification"),
                        r.get("created_at"),
                    ]
                )
            ...
            for r in rows:
                writer.writerow(
                    [
                        r.get("id"),
                        r.get("sourcing_request_id"),
                        r.get("donor_id"),
                        r.get("quantity_pledged"),
                        r.get("status"),
                        r.get("created_at"),
                    ]
                )
            ...
            for r in rows:
                writer.writerow(
                    [r.get("id"), r.get("actor_id"), r.get("action_type"), r.get("description"), r.get("created_at")]
                )
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename={table}_export_{datetime.now().strftime('%Y%m%d')}.csv"
            },
        )
    except Exception as e:
        traceback.print_exc()
