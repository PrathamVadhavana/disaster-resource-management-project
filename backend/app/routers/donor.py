...
class DonationCreate(BaseModel):
    ...
    resource_items: Optional[list] = None  # [{"resource_type": "Water", "quantity": 10, "unit": "bottles"}]
...
class DonationUpdate(BaseModel):
    ...
    resource_items: Optional[list] = None
...
async def list_approved_requests(
    ...
    try:
        ...
        needs_client_sort = sort in ("distance", "priority") or (sort == "distance" and d_lat and d_lon)
...
async def get_request_pool_donor(
    request_id: str, donor=Depends(require_donor)
):
    ...
    if body.request_id:
        ...
        if req_resp.data:
            ...
            if not donor_resource_items and body.donation_type in ("resource", "both"):
                ...
                total_requested = sum(it.get("quantity", 1) for it in request_items) if request_items else req_resp.data.get("quantity", 1)
...
async def generate_tax_certificate(
    donation_id: str, user_id: str = Depends(get_current_user_id)
):
    ...
    title_style = ParagraphStyle("CertTitle", parent=styles["Title"], fontSize=22, textColor=colors.HexColor("#065f46"), spaceAfter=4)
    subtitle_style = ParagraphStyle("CertSub", parent=styles["Normal"], fontSize=11, alignment=TA_CENTER, textColor=colors.grey, spaceAfter=20)
    heading_style = ParagraphStyle("CertHead", parent=styles["Heading2"], fontSize=13, textColor=colors.HexColor("#065f46"), spaceBefore=16, spaceAfter=8)
    normal_style = ParagraphStyle("CertNormal", parent=styles["Normal"], fontSize=10, leading=14)
    small_style = ParagraphStyle("CertSmall", parent=styles["Normal"], fontSize=8, textColor=colors.grey, alignment=TA_CENTER, spaceBefore=20)
    ...
    elements.append(Paragraph(f"Certificate No: {receipt_id}", ParagraphStyle("RefNo", parent=styles["Normal"], fontSize=10, alignment=TA_CENTER, textColor=colors.grey, spaceAfter=16)))
    ...
    if resource_items:
        ...
        res_rows = [[ri.get("resource_type", "—"), str(ri.get("quantity", 0)), ri.get("unit", "units")] for ri in resource_items]
    ...
    disclaimer_style = ParagraphStyle("Disclaimer", parent=styles["Normal"], fontSize=9, textColor=colors.HexColor("#6b7280"), leading=13)
    ...
    elements.append(Paragraph(f"Generated on {datetime.now(timezone.utc).strftime('%B %d, %Y at %H:%M UTC')}", small_style))
...
async def delete_donation(
    ...
    await db_admin.table("donations").delete().eq("id", donation_id).eq(
        "user_id", user_id
    ).async_execute()
    return {"deleted": True}
...
async def remove_pledge(disaster_id: str, user_id: str = Depends(get_current_user_id)):
    """Remove a pledge."""
    await db_admin.table("donor_pledges").delete().eq("disaster_id", disaster_id).eq(
        "donor_id", user_id
    ).async_execute()
    return {"deleted": True}
...
async def donor_stats(user_id: str = Depends(get_current_user_id)):
    ...
    total_resource_items = sum(
        sum(ri.get("quantity", 0) for ri in (d.get("resource_items") or []))
        for d in resource_donations
    )
