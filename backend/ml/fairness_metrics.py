...
class FairnessReport:
    ...
    distribution_by_vulnerability: Dict[str, Dict[str, Any]] = field(
...
def vulnerability_index(zone: ZoneDemographics) -> float:
    ...
    """
    return (
        0.4 * _clamp(zone.elderly_ratio)
        + 0.3 * _clamp(zone.children_ratio)
        + 0.3 * _clamp(zone.medical_needs_ratio)
    )
...
def historical_underservice(
    zone_id: str, records: List[HistoricalRecord], n_disasters: int = 3
) -> int:
    """
    ...
    return sum(
        1 for r in zone_records if r.resources_received < r.median_resources
    )
...
def historical_underservice_scores(
    ...
    return {
        zid: historical_underservice(zid, records, n_disasters)
        for zid in zone_ids
    }
...
def equity_score(
    ...
    zone_ids = list(
        set(vulnerability_map.keys()) & set(allocations_map.keys())
    )
    if not zone_ids:
        return round(evenness, 4)
...
def compute_fairness_report(
    ...
    underservice = historical_underservice_scores(
    ...
    mid_vuln = [
        d for d in zone_details if 0.3 <= d["vulnerability_index"] < 0.7
    ]
    ...
    def _group_summary(group: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_fulfillment_pct": round(
                sum(d["fulfillment_pct"] for d in group) / len(group), 2
            ),
        }
