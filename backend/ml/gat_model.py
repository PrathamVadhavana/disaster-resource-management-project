...
class HeteroGATEncoder(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = HeteroConv(
            {
                ("victim", "requests", "ngo"): GATv2Conv(
                    hidden, hidden, heads=heads, edge_dim=edge_dim,
        ...
        self.conv2 = HeteroConv(
            {
                ("victim", "requests", "ngo"): GATv2Conv(
                    mid, hidden, heads=heads, edge_dim=edge_dim,
...
class BilinearAssignmentHead(nn.Module):
    ...
    def forward(
        ...
        """
        Return assignment logits for every edge in ``edge_index``.

        Parameters
        ----------
        victim_emb : (num_victims, D)
        ngo_emb    : (num_ngos, D)
        edge_index : (2, E) — [victim_indices; ngo_indices]
        edge_attr  : (E, edge_dim)

        Returns
        -------
        logits : (E,) — raw scores (pre-sigmoid).
        """
...
class GATAllocator(nn.Module):
    ...
    def forward(self, data: HeteroData) -> torch.Tensor:
        ...
        logits = self.assignment_head(
            x_dict["victim"], x_dict["ngo"], edge_index, edge_attr
        )
        return logits
...
def hungarian_assignment(
    ...
    """
    Convert per-edge probabilities into a one-to-one assignment via the
    Hungarian algorithm.

    Parameters
    ----------
    data : the bipartite HeteroData graph.
    edge_probs : (E,) tensor of assignment probabilities.

    Returns
    -------
    List of ``(victim_idx, ngo_idx, prob)`` tuples for the optimal
    one-to-one matching.
    """
    edge_index = data["victim", "requests", "ngo"].edge_index
    ...
    # Build cost matrix (we *maximise* probability → *minimise* negative)
    ...
    for vi, ni in zip(row_ind, col_ind):
        if cost[vi, ni] >= 1e5:
            continue  # no feasible edge existed
        assignments.append((int(vi), int(ni), float(-cost[vi, ni])))
...
def explain_assignment(
    ...
    """
    Return the top-k features that most influenced a specific assignment.

    Uses a gradient-based approximation (input × gradient) as a fast
    surrogate for SHAP values.  For full SHAP, set ``use_shap=True``
    (requires the ``shap`` package and is slower).

    Parameters
    ----------
    model : trained GATAllocator.
    data  : the graph used for inference.
    victim_idx, ngo_idx : indices of the assigned pair.
    top_k : how many features to report.

    Returns
    -------
    List of dicts with keys ``feature``, ``importance``, ``value``.
    """
    victim_feature_names = [
        "lat", "lon", "priority_score", "medical_needs",
        "hours_since_request",
        *[f"rtype_{rt}" for rt in ["food", "water", "medical", "shelter",
                                    "clothing", "financial_aid", "evacuation", "volunteers"]],
    ]
    ngo_feature_names = [
        "lat", "lon", "capacity_score",
        *[f"avail_{rt}" for rt in ["food", "water", "medical", "shelter",
                                    "clothing", "financial_aid", "evacuation", "volunteers"]],
        "avg_response_time", "current_load_ratio",
    ]
    edge_feature_names = ["distance_km", "travel_time_min", "match_score"]

    model.eval()

    # Clone data tensors and enable gradients
    ...
    # Find the edge for (victim_idx, ngo_idx)
    ...
    # Compute input × gradient importance
...
def save_checkpoint(model: GATAllocator, path: Path | str | None = None) -> Path:
