            ...
            >
                ...
                <div className="flex items-center justify-between h-16 px-5 border-b border-slate-200 dark:border-white/5 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-[10px] text-slate-400 dark:text-slate-500 uppercase tracking-widest">Victim Dashboard</p>
                        </div>
                    </div>
                    <button
                        onClick={() => setCollapsed(false)}
                        className="lg:hidden p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 transition-colors"
                    >
                ...
                <div className="p-4 border-b border-slate-200 dark:border-white/5 shrink-0">
                    <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-50 dark:bg-white/[0.03] border border-slate-200 dark:border-white/5">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-red-500/80 to-orange-500/80 flex items-center justify-center text-white font-bold text-sm shadow-lg">
                            {profile?.full_name?.charAt(0)?.toUpperCase() || '🆘'}
                        </div>
                        <div className="min-w-0">
                            <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">
                                {profile?.full_name || 'Victim User'}
                            </p>
                ...
                <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
                    <p className="px-3 pt-2 pb-2 text-[10px] text-slate-400 dark:text-slate-600 uppercase tracking-widest font-semibold">
                        Navigation
                    </p>
                    {navItems.map((item) => {
                        ...
                        return (
                            <Link
                                ...
                                className={cn(
                                    ...
                                    active
                                        ? 'bg-gradient-to-r from-red-500/10 to-orange-500/10 dark:from-red-500/15 dark:to-orange-500/10 text-red-700 dark:text-white border border-red-200 dark:border-red-500/20 shadow-sm dark:shadow-lg dark:shadow-red-500/5'
                ...
                <div className="p-3 border-t border-slate-200 dark:border-white/5 space-y-1 shrink-0">
                    {/* Theme toggle - only render icon on client to avoid hydration error */}
                    <button
